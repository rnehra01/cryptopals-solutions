...
if __name__ == '__main__':
	...
	original = MT19937(randint(0,2**32-1))
	random_numbers = [original.random() for x in range(624)]

	initial_seed = [MT19937.untemper(x) for x in random_numbers]
	# New random numbers from the original instance
	new_random_numbers = [original.random() for x in range(624)]
	
	attack = MT19937(initial_seed)
	guessed_random_numbers = [attack.random() for x in range(624)]
	
	if guessed_random_numbers == new_random_numbers : 
		print '[+] Test Passed : guessed_random_numbers == new_random_numbers' 
	else :
		print '[-] Test Failed : guessed_random_numbers == new_random_numbers'
